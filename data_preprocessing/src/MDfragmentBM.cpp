...
void MDfragmentBM::SetPreviousSpill(bool prSpillEx, unsigned int prSpill) {
  ...
  if (prSpill >= 65535)_previousSpillTag = 0; 
}
...
void MDfragmentBM::Init(int &spillTag) {
      ...
      if (dw.GetDataType() != MDdataWordBM::SpillTrailer2) {
        ...
        while (!done) {
          ...
          if (dw.GetDataType() == MDdataWordBM::TrigHeader) {
            ...
            _boardId = dw.GetBoardId();
            spillTag = _spillTag;
            // cout   <<"1: BM spill Header \"A\" Board ID "<< _boardId <<" SpillTag: "<< spillTag <<endl;
            ++ptr;
            dw.SetDataPtr(ptr);
            _size += 4;
          } 
            ...
            if (!_previousSpillTagExist){
              ...
              if (_spillTag != _previousSpillTag +1){
                // spillTag = _spillTag;
                if (spillTag != _previousSpillTag +1){
                  cout << "ERROR in MDfragmentBM::Init() : No events for Spill Tag : " <<_previousSpillTag + 1<<endl;
                  //if (_spillTag > _previousSpillTag){
                  //    _previousSpillTag = _spillTag;
                  //}
                  //account for skipped spill near wraparound spill value 
                  if (_previousSpillTag >= 65533){
                    _previousSpillTag = _spillTag;
                  }
                  else {
                    // cout << _previousSpillTag << " " << _spillTag << endl;
                    throw MDexception("ERROR in MDfragmentBM::Init() :  Spill Tag is less than previous Spill Tag.");
                  }
                }
                else 
                  _spillTag = spillTag;
            ...
            if (dw.GetDataType() == MDdataWordBM::SpillTrailer1 && dw.GetSpillHeadId() == 0) {
              ...
              throw MDexception("ERROR in MDfragmentBM::Init(1) : Wrong data type.");
            }
            ...
            if (dw.GetDataType() == MDdataWordBM::SpillTrailer1 && dw.GetSpillHeadId() == 1) {
              ...
              throw MDexception("ERROR in MDfragmentBM::Init(2) : Wrong data type.");
            }
            ...
            if (dw.GetDataType() == MDdataWordBM::SpillTrailer2 ){
              ...
              throw MDexception("ERROR in MDfragmentBM::Init(3) : Wrong data type.");
            }
