...
int main(int argc, char **argv) {
  ...
  int NumberOfEB = 30;
  int len = 18;
  //vector<int> FEBs = {0,1,2,3,4,8,9,10,11,12,16,17,18,19,20,24,25,26,27};
  //vector<int> FEBs = {0,1,2,3,4,8,9,10,11,12,16,17,18,19,20,24,25,26,27};
  ...
  if (USJ){
    ...
    len = 24;
    FEBs.clear();
    FEBs = {0,1,2,3,4,8,9,10,11,12,16,17,18,19,20,24,25,26,27,56,57,58,59,60};
  }
...
*/
  ...
  //loop over spills
  for (Int_t subSpill = 0; subSpill<(TotalSpills-1)/10.; subSpill++) {
    Int_t micropulse=0;
    ...
    if (!SpillMissed)
    {
      for ( int TOFtrigger = 0; TOFtrigger < FEB[12].FEBSN->size(); TOFtrigger++){
        ...
        if (sDataType == "LANL20" || sDataType == "LANL90" || sDataType == "USJ"){
          if (FEB[12].hitsChannel->at(TOFtrigger) != 30 && FEB[12].hitsChannel->at(TOFtrigger) != 31)
  	       continue;
        }
	  ...
	  for (int ij = 0; ij < 48; ij++ ){
          ...
          Int_t GTindex[2] = {0,0};
          for (int i = 0; i < len; i++){
            cout<<"in "<<i<<" th board, id : "<<FEBs[i]<<endl;
            if (FEBs[i]!=12){

              //cout<<"hit size , gtrig at toftrigger : "<<FEB[FEBs[i]].FEBSN->size()<<" "<<FEB[FEBs[i]].GTrigTag->at(TOFtrigger)<<endl;
	      ...
	      //cout<<"gtrig begin : "<<bounds.first - FEB[FEBs[i]].GTrigTag->begin()<<" "<<FEB[FEBs[i]].hitAmpl->at(bounds.first - FEB[FEBs[i]].GTrigTag->begin())<<endl;
              //cout<<"gtrig end : "<<bounds.second - FEB[FEBs[i]].GTrigTag->begin()<<" "<<FEB[FEBs[i]].hitAmpl->at(bounds.second - FEB[FEBs[i]].GTrigTag->begin() -1)<<endl;

	      cout<<"chck point 4 : "<<GTindex[0]<<" "<<GTindex[1]<<endl;
              for (int check = GTindex[0]; check <  GTindex[1] ; check++){

		cout<<"check point 4.5 : "<<FEB[FEBs[i]].hitTimefromSpill->at(check) - FEB[12].hitTimefromSpill->at(TOFtrigger)<<endl;
	    ...
	    cout<<"check point 10"<<endl;
          ...
          event->SetFEB12LeadTime(FEB[0].hitLeadTime->at(TOFtrigger));
          event->SetFEB12hitTfromSpill(FEB[0].hitTimefromSpill->at(TOFtrigger));
