...
int main(int argc, char **argv) {
  ...
  for(Int_t subSpill = 0; subSpill< t.GetEntries(); subSpill++){
    ...
    double Nphot = edep * EdepToPhotConv_FGD;
    ...
    if ((int)neutronHitX[locSave]%2 == 0) {
      xNphot = Nphot * ( a*exp((-(TMath::Abs(aveXSave-xx)))/LongAtt) + (1-a)*exp((-(TMath::Abs(aveXSave-xx)))/ShortAtt) );
      yNphot = Nphot * ( a*exp((-(TMath::Abs(aveYSave-yy)))/LongAtt) + (1-a)*exp((-(TMath::Abs(aveYSave-yy)))/ShortAtt) );
      zNphot = Nphot * ( a*exp((-(TMath::Abs(aveZSave-zz)))/LongAtt) + (1-a)*exp((-(TMath::Abs(aveZSave-zz)))/ShortAtt) );
