...
int main( int argc, char **argv ) {
  ...
  // hack in here Dec 1 2020
  string rootFileOutput=GetLocation(vFileNames[0].c_str());
  rootFileOutput+="_raw.root";
  //string rootFileOutput="temp_raw.root";
  ...
  for (vector<string>::iterator itFileName=vFileNames.begin(); itFileName != vFileNames.end(); itFileName++) {
    ...
    argh.AddArgument("maxSpills","Maximum number of spills to process","z","<int>","-1");
    ...
    if ( argh.GetValue("maxSpills", maxSpills) != MDARGUMENT_STATUS_OK ) return -1;
          ...
          for (int i=0; i<nTr; ++i) {
            ...
            for (int ich=0; ich<BM_FEB_NCHANNELS; ++ich) {
              ...
              for (unsigned int ih=0; ih<nlHits; ++ih) {
                ...
                if (FEB[spill.GetBoardId()].SpillTimeGTrig.back() > 942){
