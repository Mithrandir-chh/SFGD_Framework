...
int main(int argc, char **argv) {
  ...
  t.Add("~/work/sfgd_framework/data_preprocessing/build/neutron/FHC_*_24x8x48_wFiber_wDead_realSize_wTyvek_noGap_realRot_180_*.root");
...
*/
  ...
  Float_t neutronPx[1000], neutronPy[1000], neutronPz[1000];
  ...
  int timeFlagXY[20000]={};
  int timeFlagXZ[20000]={};
  int timeFlagYZ[20000]={};
  int timeFlag[20000]={};
  ...
  TFile wfile(Form("LANL_MC_normalSize_v1_realRot_180_r%f.root",atof(argv[1])), "recreate");
  //TFile wfile("LANL_MC_normalSize_pencilBeam_100MeV.root", "recreate");

  //loop over spills
  for (Int_t subSpill = 0; subSpill<t.GetEntries()/3; subSpill++) {
    ...
    for (int veryTemp =0; veryTemp<1000; veryTemp ++){
      ...
      neutronHitZ[veryTemp] = TMath::Cos(rotation_x) * (neutronHitZ[veryTemp] + detHalfSizeZ) - detHalfSizeZ;
    }
