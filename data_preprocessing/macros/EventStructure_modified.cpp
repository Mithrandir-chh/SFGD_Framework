...
int main(int argc, char **argv) {
  ...
  if (USJ){
    ...
    len = 24;
    FEBs.clear();
    FEBs = {0,1,2,3,4,8,9,10,11,12,16,17,18,19,24,25,26,27,56,57,58,59,60,61};
  }
    ...
    if (!SpillMissed){
      ...
      for ( int TOFtrigger = 0; TOFtrigger < FEB[12].FEBSN->size(); TOFtrigger++){
        ...
        if (sDataType == "LANL20" || sDataType == "LANL90" || sDataType == "USJ"){
          if (FEB[12].hitsChannel->at(TOFtrigger) != 0) 
            continue;
        }
	  ...
	  //if(TOFtrigger > 0)
	  //std::cout<<"testing "<<TOFtrigger<<" "<<FEB[12].hitTimefromSpill->at(TOFtrigger)<<" "<<FEB[12].hitTimefromSpill->at(TOFtrigger)-FEB[12].hitTimefromSpill->at(TOFtrigger-1)<<std::endl;
		...
		//if (passMycut )
		//if (passMycut || TOFtrigger == FEB[12].FEBSN->size()-1)
		{
