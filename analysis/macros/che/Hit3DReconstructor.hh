#ifndef HIT3D_RECONSTRUCTOR_H
#define HIT3D_RECONSTRUCTOR_H
...
// CRTP Base Class - Framework Independent
template<typename Derived>
class Hit3DReconstructorCRTP {
public:
    // Main reconstruction interface - delegates to derived class
    std::vector<Hit3D> ReconstructHits(void* event) {
        return static_cast<Derived*>(this)->DoReconstructHits(event);
    }
    
    // Static algorithm functions - no framework dependencies
    static std::vector<Hit3D> ResolveAdjacentHits(const std::vector<Hit3D>& initialHits, 
                                                  const ViewHits& viewXY, 
                                                  const ViewHits& viewXZ, 
                                                  const ViewHits& viewZY);
    
    static std::vector<Hit3D> FindThreeViewMatches(const ViewHits& viewXY, 
                                                   const ViewHits& viewXZ, 
                                                   const ViewHits& viewZY,
    ...
    static std::vector<Hit3D> FindTwoViewMatches(const ViewHits& viewXY, 
                                                 const ViewHits& viewXZ, 
                                                 const ViewHits& viewZY,
                                                 std::vector<bool>& usedXY, 
                                                 std::vector<bool>& usedXZ, 
    ...
    static double GetViewCharge(const std::string& view, double x, double y, double z, 
                               const ViewHits& viewXY, const ViewHits& viewXZ, const ViewHits& viewZY);
    
    static double GetViewTiming(const std::string& view, double x, double y, double z, 
                               const ViewHits& viewXY, const ViewHits& viewXZ, const ViewHits& viewZY);
    
    static Hit3D FindEntryPoint(const std::vector<Hit3D>& allHits);

protected:
    // Protected destructor - this is a CRTP base class
    ~Hit3DReconstructorCRTP() = default;
};
...
// Copy your implementations from the .cpp file here:
template<typename Derived>
std::vector<Hit3D> Hit3DReconstructorCRTP<Derived>::FindThreeViewMatches(const ViewHits& viewXY, 
                                                                         ...
                                                                         std::vector<bool>& usedXZ, 
                                                                         std::vector<bool>& usedZY) {
    // Copy the implementation from your .cpp file here
