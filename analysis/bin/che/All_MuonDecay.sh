...
DESTINATION_DIR="/media/disk_b/standard_software/sfgd_framework/cosmic_data_2025/che_side0/Total/MuonDecay_000_to_020_calibrated"
...
process_subfolder() {
    local subfolder=$1
    local dest_dir=$2 # Receive the destination directory as an argument
    ...
    for d in "${subfolder}/"*root_MuonDecay; do
        if [ -d "$d" ]; then
