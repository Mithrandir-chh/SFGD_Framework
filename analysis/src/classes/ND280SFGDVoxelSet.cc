...
//******************************************************************************
    ...
    hXY->SetTitle(";X position [cm];Y position [cm]");     
    TH2F *hXZ = new TH2F(hXZname.Data(),hXZname.Data(), 220, -0., 220, 220, 0., 220);
    hXZ->SetTitle(";X position [cm];Z position [cm]");   
    TH2F *hZY = new TH2F(hYZname.Data(),hYZname.Data(), 220, -0., 220, 220, 0., 220);
    hZY->SetTitle(";Z position [cm];Y position [cm]");
    ...
    if(opt!=1){
        ...
        hZY->GetZaxis()->SetRangeUser(0.1,1.05*Qmax3);
        ...
        cc->cd(3);
        hZY->Draw("COLZ");
        hZY->GetXaxis()->SetRangeUser(minZ-tolerance,maxZ+tolerance);
        hZY->GetYaxis()->SetRangeUser(minY-tolerance,maxY+tolerance);
    }
    ...
    // draw only the voxels
        ...
        cc->cd(4);
        cc->Update();
        //cc->SaveAs("/home/ws1216/Documents/SFGD/data/LANL_2019_neutron/MC/EventDisplay_PNG/"+canvName+".png");
        this->Draw3D(wait,color);
    }
...
//********************************************************************
    ...
    fData->Draw("y:x:z:color","","box2");
    TH3F *htemp = (TH3F*)gPad->GetPrimitive("htemp");
    htemp->GetZaxis()->SetLimits(minY-tolerance,maxY+tolerance);
    htemp->GetYaxis()->SetLimits(minX-tolerance,maxX+tolerance);
